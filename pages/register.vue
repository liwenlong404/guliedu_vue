  ...
  <div class="main">
    ...
    <div class="sign-up-container">
      <el-form ref="userForm" :model="params">
        ...
        <el-form-item class="input-prepend restyle no-radius" prop="mobile" :rules="[{ required: true, message: '请输入邮箱', trigger: 'blur' }]">
          <div>
            <el-input type="text" placeholder="邮箱号" v-model="params.mail"/>
            <i class="iconfont icon-phone"/>
          </div>
        </el-form-item>
        ...
        <el-form-item class="input-prepend restyle no-radius" prop="code" :rules="[{ required: true, message: '请输入验证码', trigger: 'blur' }]">
          <div style="width: 100%;display: block;float: left;position: relative">
            <el-input type="text" placeholder="邮箱验证码" v-model="params.code"/>
            <i class="iconfont icon-phone"/>
          </div>
...
export default {
  ...
  methods: {
    ...
    //通过输入邮箱发送验证码
    getCodeFun(){
      register.sendCode(this.params.mail)
      .then(response=>{
        //是否发送置为false
        this.sending=false;
        //调用倒计时，倒计时接收，sending置为true
        this.timeDown()
      })
    },
